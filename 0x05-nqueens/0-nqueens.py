...
def solveNQueens(n):
    ...
    """
    # This will store all the solutions in terms of column positions for each row
    solutions = []
    ...
    def backtrack(row):
        ...
        if row == n:
            solutions.append(current_solution[:])  # Store a copy of the solution
            return
        
        for col in range(n):
            # Check if the column or diagonals are attacked
            if attacked_columns[col] or attacked_diag1[row - col + n - 1] or attacked_diag2[row + col]:
                continue
            
            # Place queen on the current row, column
            current_solution[row] = col
            attacked_columns[col] = attacked_diag1[row - col + n - 1] = attacked_diag2[row + col] = True
            
            # Move to the next row
            backtrack(row + 1)
            
            # Backtrack, remove queen and reset attacks
            attacked_columns[col] = attacked_diag1[row - col + n - 1] = attacked_diag2[row + col] = False
